...
const EditFeatureService = ({
  ...
  handleEditFeatureService,
}) => {
  const [title, setTitle] = useState(selectedFeatureService.title);
  const [details, setDetails] = useState(selectedFeatureService.details);
  const [files, setFiles] = useState([]);
  const [image, setImage] = useState(null);

  useEffect(() => {
    // Preload the selected category's image in the files array
    if (selectedFeatureService.image) {
      setImage(selectedFeatureService.image); // Preload the image URL
    }
  }, [selectedFeatureService]);
  ...
  const handleSubmit = (e) => {
    ...
    // Update localStorage
    const existingFeatureServices =
      JSON.parse(localStorage.getItem("featureServiceData")) || [];
    const updatedFeatureServices = existingFeatureServices.map((category) =>
      category.id === updatedFeatureService.id
    ...
    handleEditFeatureService(updatedFeatureService);
  };
  ...
  return (
    <>
      <div className="flex items-center gap-3 mb-3">
        <div>
          <h1 className="text-xl font-bold">Edit FeatureService</h1>
          <p className="text-sm text-gray-500">
            You can edit FeatureService details from here.
          </p>
        </div>
      </div>
...
export default EditFeatureService;
