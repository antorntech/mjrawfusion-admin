...
const AddFeatureService = ({ handleAddFeatureService, isAdding }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!image) {
      toast.info("Please upload an image for the feature service.");
      return;
    }
    ...
    localStorage.setItem(
      "featureServiceData",
      JSON.stringify([...existingFeatureServices, newFeatureService])
    );

    handleAddFeatureService(newFeatureService);
    setTitle("");
    setDetails("");
    setImage(null);
  ...
  return (
    <>
      <div className="flex items-center gap-3 mb-3">
        <div>
          <h1 className="text-xl font-bold">Add Feature Service</h1>
          <p className="text-sm text-gray-500">
            You can add FeatureService details from here.
          </p>
        </div>
      </div>
      <form onSubmit={handleSubmit} className="form">
        ...
        {isAdding ? (
          ...
          <Tooltip content="Don't add more than three.">
            <button
              type="submit"
...
export default AddFeatureService;
