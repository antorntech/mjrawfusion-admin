...
const EditClient = ({ selectedClient, handleEditClient }) => {
  ...
  useEffect(() => {
    // Preload the selected client's image in the files array
    if (selectedClient.image) {
      setImage(selectedClient.image); // Preload the image URL
    }
  }, [selectedClient]);
  ...
  const handleSubmit = (e) => {
    ...
    const updatedCategories = existingCategories.map((client) =>
      client.id === updatedClient.id ? updatedClient : client
    );
    localStorage.setItem("clientsData", JSON.stringify(updatedCategories));
      ...
      <div className="flex items-center gap-3 mb-3">
        <div>
          ...
          <p className="text-sm text-gray-500">
            You can edit client details from here.
          </p>
        </div>
      </div>
