...
const EditPrinciple = ({ selectedPrinciple, handleEditPrinciple }) => {
  const date = moment().format("Do MMM, YYYY");
  const [name, setName] = useState(selectedPrinciple.name);
  const [files, setFiles] = useState([]);
  const [image, setImage] = useState(null); // Separate state for the main image

  useEffect(() => {
    // Preload the selected principle's image in the files array
    if (selectedPrinciple.image) {
      setImage(selectedPrinciple.image); // Preload the image URL
    }
  }, [selectedPrinciple]);
  ...
  const handleSubmit = (e) => {
    ...
    const updatedCategories = existingCategories.map((principle) =>
      principle.id === updatedPrinciple.id ? updatedPrinciple : principle
    );
    localStorage.setItem("principlesData", JSON.stringify(updatedCategories));

    handleEditPrinciple(updatedPrinciple);
  };
  ...
  return (
    <>
      <div className="flex items-center gap-3 mb-3">
        <div>
          <h1 className="text-xl font-bold">Edit Principle</h1>
          <p className="text-sm text-gray-500">
            You can edit principle details from here.
          </p>
        </div>
      </div>
